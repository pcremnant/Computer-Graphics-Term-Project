...
CObject_Obstacle::CObject_Obstacle(CCamera& cam, glm::vec3 size, glm::vec3 pos, glm::mat4 proj) : CObject(cam, pos, proj) {
	vec3_Size = size;
	int_Type = COLLISION_OBSTACLE;
	vector_Model.emplace_back(std::make_unique<CModel_cube>(LAYOUT_UV, size));
...
std::vector<float> CObject_Obstacle::GetBoundingBox() {
	// xz 평면 기준 좌표 (y값은 0) - 앞뒤 / 좌우 회전이 생긴다면 수정 요청할 것
	// front_right - back_right - back_left - front_left
	glm::vec3 vertex[8];
	vertex[0] = glm::vec3{ vec3_Size.x , vec3_Size.y ,vec3_Size.z };
...
void CObject_Obstacle::Collide(int type) {
	switch (type) {
	case COLLISION_ENEMY:
